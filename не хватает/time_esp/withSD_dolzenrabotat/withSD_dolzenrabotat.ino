...
boolean wake_flag, move_arrow, humflag = 1, esp_servo_wake_flag = 0;
...
servotempday.attach(A4);
servohumesp.attach(A5);
servohum.attach(7);
servotemp.attach(6);
...
esp8266.println("lol");
while(!esp8266.find("Date:") && !myTimer.isReady()) {}
while (!myTimer.isReady() && esp8266.available() && !esp8266.find("GMT")) {
          times += esp8266.readString();
